...
mod tests {
    ...
    mod external_sig {
        ...
        #[test]
        fn test_cosmos_sig_verify_eth() {
            let deps = mock_dependencies();
            let signature_raw = Binary::from_base64("eyJwdWJfa2V5IjoiQTdDOTBNVTI4MzRnUEZhNk9ZMVR6V3ZQU25WcXJZUThpWTVCaVZ0d1RIbE8iLCJzaWduYXR1cmUiOiJQak1KeE5EL1dxeFMxWDVwLzBUcVJHSnMxUll2cEVWeUhmV2J1NGhzYnVZTC9uRUt5d0ZIWFBNczA4SmhtUjEyQnNnbjA4bW1lS1p6UEJKaEFuSWhQQT09In0=");

            let sig = SignatureInfo {
                claim_msg: Binary::from_base64("eyJldGgiOiIweDI3ZGQ0MzI4QjlkRDk5MjgxZDM5ODA1MEI1OEVBMGRiRWMxODFFNTYiLCJiYWJ5Ijoid2FzbTFxNGx5NXJtNmpwZmhoeDczeHZsbHBjNnVjbXIzeHJyNTBkdjVrcyJ9").unwrap(),
                signature: signature_raw.unwrap(),
            };
            let cosmos_signature: CosmosSignature = from_binary(&sig.signature).unwrap();
            let res = cosmos_signature
        ...
        fn test_cosmos_eth_recover() {
            ...
            let cosmos_signature: CosmosSignature = from_binary(&sig.signature).unwrap();
            let res = cosmos_signature
                .derive_eth_addr_from_sig(deps.as_ref(), &sig.claim_msg,0)
                .unwrap();
            println!("eth recover: {:?}", res);  
            let expected = sig.extract_behalf_addr().unwrap();  
            println!("expected.0: {:?}", expected.0);  
            assert_eq!(res.to_lowercase(), expected.0.to_lowercase());
        }
